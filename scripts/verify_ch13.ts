...
import { EL_PHY12_CH13_QUESTIONS } from '../src/data/el_phy12_chapter13_data';
import katex from 'katex';

async function main() {
  console.log(`EL_PHY12_CH13_QUESTIONS length: ${EL_PHY12_CH13_QUESTIONS.length}`);
  
  if (EL_PHY12_CH13_QUESTIONS.length !== 200) {
    throw new Error(`Expected 200 questions, got ${EL_PHY12_CH13_QUESTIONS.length}`);
  }

  const counts: Record<string, number> = { A: 0, B: 0, C: 0, D: 0 };
  for (const q of EL_PHY12_CH13_QUESTIONS) {
  ...
  const normalQuestions = await MathService.getQuestionsByChapter('el-phy12-ch13', 'all', 'Elementary Physics', 'Normal');
  ...
  const advancedQuestions = await MathService.getQuestionsByChapter('el-phy12-ch13', 'all', 'Elementary Physics', 'Advanced');
  console.log(`MathService advanced questions count: ${advancedQuestions.length}`);

  const normalCountByService = MathService.getQuestionCountByDifficulty(12, 'el-phy12-ch13', 'Normal', 'Elementary Physics');
  console.log(`MathService getQuestionCountByDifficulty (Normal): ${normalCountByService}`);

  const advancedCountByService = MathService.getQuestionCountByDifficulty(12, 'el-phy12-ch13', 'Advanced', 'Elementary Physics');
  ...
  if (
    ...
    console.log('🎉 CHAPTER 13 (NUCLEAR PHYSICS) VERIFICATION PASSED PERFECTLY!');
  } else {
