...
import { EL_PHY12_CH14_QUESTIONS } from '../src/data/el_phy12_chapter14_data';
import katex from 'katex';

async function main() {
  console.log(`EL_PHY12_CH14_QUESTIONS length: ${EL_PHY12_CH14_QUESTIONS.length}`);
  
  if (EL_PHY12_CH14_QUESTIONS.length !== 200) {
    throw new Error(`Expected 200 questions, got ${EL_PHY12_CH14_QUESTIONS.length}`);
  }

  const counts: Record<string, number> = { A: 0, B: 0, C: 0, D: 0 };
  for (const q of EL_PHY12_CH14_QUESTIONS) {
    ...
    if (new Set(opts).size < 4) {
      throw new Error(`Question ${q.id} has duplicate options: ${JSON.stringify(opts)}`);
    }
  }
  console.log('Answer distribution in raw data:', counts);

  const normalQuestions = await MathService.getQuestionsByChapter('el-phy12-ch14', 'all', 'Elementary Physics', 'Normal');
  ...
  const advancedQuestions = await MathService.getQuestionsByChapter('el-phy12-ch14', 'all', 'Elementary Physics', 'Advanced');
  console.log(`MathService advanced questions count: ${advancedQuestions.length}`);

  const normalCountByService = MathService.getQuestionCountByDifficulty(12, 'el-phy12-ch14', 'Normal', 'Elementary Physics');
  console.log(`MathService getQuestionCountByDifficulty (Normal): ${normalCountByService}`);

  const advancedCountByService = MathService.getQuestionCountByDifficulty(12, 'el-phy12-ch14', 'Advanced', 'Elementary Physics');
  ...
  if (
    ...
    console.log('🎉 CHAPTER 14 (PARTICLE PHYSICS) VERIFICATION PASSED PERFECTLY!');
  } else {
